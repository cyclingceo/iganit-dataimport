...
class BulkImporter(BulkBlockImporter):
    def fetch(self):
        file_path = '/home/jayramj/projects/iganit/masterdata-mgmt/downloads/bulkblock/NSE_Bulk.csv'
        field_names = ['trading_date', 'symbol', 'security_name', 'client_name', 'trade_type', 'quantity_traded', 'trade_price',
                       'remarks', 'exchange', 'deal_type']
        insert_sql = 'INSERT INTO public.bulk_block_deals (isin, trading_date, client_name, trade_type, quantity_traded,' \
                       ...
                       'from master_stock_list where nse_symbol = %s'
        ...
        for record in csv_data:
            ...
            cur.execute(insert_sql,[sd.get('trading_date'), sd.get('client_name'), sd.get('trade_type'),
...
class BlockImporter(BulkBlockImporter):
    def fetch(self):
        file_path = '/home/jayramj/projects/iganit/masterdata-mgmt/downloads/bulkblock/NSE_Block.csv'
        field_names = ['trading_date', 'symbol', 'security_name', 'client_name', 'trade_type', 'quantity_traded', 'trade_price',
                       'remarks', 'exchange', 'deal_type']
        insert_sql = 'INSERT INTO public.bulk_block_deals (isin, trading_date, client_name, trade_type, quantity_traded,' \
                       ' trade_price, exchange, deal_type) SELECT isin , %s, %s, %s, %s, %s, %s, %s ' \
